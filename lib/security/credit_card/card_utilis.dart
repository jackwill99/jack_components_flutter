...
/// void getTypeFrm() {
///   /// within 6 digit we can define
///   if (controller.text.length <= 6) {
///     String cardNum = CardUtils.getCleanedNumber(controller.text);
///     CardType type = CardUtils.getCardTypeFrmNumber(controller.text);
///     if (type != cardType) {
///       setState(() {
///         cardType = type;
///       });
...
class JackCardUtils {
  ...
  static Widget? getCardIcon(JackCardType? cardType) {
    String img = "";
    Icon? icon;
    switch (cardType) {
      case JackCardType.Master:
        img = 'mastercard.png';
        break;
      case JackCardType.Visa:
        img = 'visa.png';
        break;
      case JackCardType.Verve:
        img = 'verve.png';
        break;
      case JackCardType.AmericanExpress:
        img = 'american_express.png';
        break;
      case JackCardType.Discover:
        img = 'discover.png';
        break;
  ...
  static JackCardType getCardTypeFrmNumber(String input) {
    JackCardType cardType;
    if (input.startsWith(RegExp(
        r'((5[1-5])|(222[1-9]|22[3-9][0-9]|2[3-6][0-9]{2}|27[01][0-9]|2720))'))) {
      cardType = JackCardType.Master;
    } else if (input.startsWith(RegExp(r'[4]'))) {
      cardType = JackCardType.Visa;
    } else if (input.startsWith(RegExp(r'((506(0|1))|(507(8|9))|(6500))'))) {
      cardType = JackCardType.Verve;
    } else if (input.startsWith(RegExp(r'((34)|(37))'))) {
      cardType = JackCardType.AmericanExpress;
    } else if (input.startsWith(RegExp(r'((6[45])|(6011))'))) {
      cardType = JackCardType.Discover;
